...
static ssize_t proc_read(struct file *file, char __user *usr_buf, size_t count, loff_t *pos)
{
	...
	/* If tsk pid is vaild, then print its infos: */
	if (tsk)
	{
		rv = snprintf(buffer,
					  BUFFER_SIZE, "command = [%s], pid = [%d], state = [%ld]\n",
...
static ssize_t proc_write(struct file *file, const char __user *usr_buf, size_t count, loff_t *pos)
{
	...
	kstrtoint(k_mem, 10, &l_pid);
	printk(KERN_INFO "Set current PID to %d", l_pid);
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Linux Kernel Module for Task Infomation");